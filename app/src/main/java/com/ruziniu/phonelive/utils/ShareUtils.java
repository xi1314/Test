...
public class ShareUtils {
    ...
    public static void share(Context context, int id, UserBean user) {
        switch (id) {
    ...
    //录播分享
    public static void share1(Context context, int id, UserBean user) {
        switch (id) {
            case R.id.ll_live_shar_qq:
                share1(context, 3, user, null);
                break;
            case R.id.ll_live_shar_pyq:
                share1(context, 2, user, null);
                break;
            case R.id.ll_live_shar_qqzone:
                share1(context, 4, user, null);
                break;
            case R.id.ll_live_shar_sinna:
                share1(context, 0, user, null);
                break;
            case R.id.ll_live_shar_wechat:
                share1(context, 1, user, null);
                break;
        }
    }
    ...
    public static void share1(final Context context, final int index, final UserBean user, final PlatformActionListener listener) {
        String[] names = new String[]{SinaWeibo.NAME, Wechat.NAME, WechatMoments.NAME, QQ.NAME, QZone.NAME};
        String content = "";
        if (user.getUpload()==2){
            content = user.getUser_nicename() + "的上传视频";
        }else{
            content = user.getUser_nicename() + "的录播回放";
        }
        switch (index) {
            case 0: {
                share1(context, names[0], false, content, user, listener);
                break;
            }
            case 1: {
                share1(context, names[1], false, content, user, listener);
                break;
            }
            case 2: {
                share1(context, names[2], false, content, user, listener);
                break;
            }
            case 3: {
                share1(context, names[3], false, content, user, listener);
                break;
            }
            case 4: {
                share1(context, names[4], false, content, user, listener);
                break;
            }
        }
    ...
    public static void share1(final Context context, String name, boolean showContentEdit, String content, final UserBean user, PlatformActionListener listener) {
        ...
        oks.setTitle(user.getUser_nicename());
        ...
        if (user.getUpload()==2){
            oks.setUrl("http://ruziniu.yunbaozhibo.com/wxshare/Phone/videoback?id="+user.getId());
            oks.setSiteUrl("http://ruziniu.yunbaozhibo.com/wxshare/Phone/videoback?id="+user.getId());
            oks.setTitleUrl("http://ruziniu.yunbaozhibo.com/wxshare/Phone/videoback?id="+user.getId());
        }else{
            oks.setUrl("http://ruziniu.yunbaozhibo.com/wxshare/Phone/liveback?id="+user.getId());
            oks.setSiteUrl("http://ruziniu.yunbaozhibo.com/wxshare/Phone/liveback?id="+user.getId());
            oks.setTitleUrl("http://ruziniu.yunbaozhibo.com/wxshare/Phone/liveback?id="+user.getId());
        }
